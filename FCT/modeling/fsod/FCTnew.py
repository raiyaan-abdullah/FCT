...
class BA_CISA(nn.Module):
    ...
    def forward(self, im_data, support_ims, all_cls_gt_boxes=None):
        ...
        #         kv_y = self.kv(y_).reshape(B_y, -1, 2, self.num_heads, C_y // self.num_heads).permute(2, 0, 3, 1, 4)
        #     else:
        #         kv_x = self.kv(x).reshape(im_info = im_info.data
        ...
        return dense_support_feature
       # return x, y #torch.cat((x, y), dim=1)


class Block(nn.Module):

    def __init__(self,classes, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        self.ba_cisa = BA_CISA(classes, attention_type = 'concat', rpn_reduce_dim=256, rcnn_reduce_dim=256, gamma=0.1, semantic_enhance=False, n_way=2, n_shot=5, pos_encoding=True)
    ...
    def forward(self, x, H_x, W_x, y, H_y, W_y):
        x = self.ba_cisa(im_data=x, support_ims=y)
        print(f"X = {x.size()}, Heignt = {H_x}")
...
def make_stage(i,
    ...
    block = nn.ModuleList([Block(
        classes = classes, dim=embed_dims[i], num_heads=num_heads[i], mlp_ratio=mlp_ratios[i], qkv_bias=qkv_bias,
        qk_scale=qk_scale,
