...
def _get_vhbt_fewshot_instances_meta():
    ret = {
        "thing_classes": vhbt_CATEGORIES,
        "novel_classes": vhbt_NOVEL_CATEGORIES,
        "base_classes": vhbt_BASE_CATEGORIES,
    }
    return ret

def _get_vhbt_instances_meta():
    thing_ids = [k["id"] for k in vhbt_CATEGORIES if k["isthing"] == 1]
    thing_colors = [k["color"] for k in vhbt_CATEGORIES if k["isthing"] == 1]
    assert len(thing_ids) == 15, len(thing_ids)
    # Mapping from the incontiguous vhbt category id to an id in [0, 14]
    thing_dataset_id_to_contiguous_id = {k: i for i, k in enumerate(thing_ids)}
    thing_classes = [k["name"] for k in vhbt_CATEGORIES if k["isthing"] == 1]
    ret = {
...
def _get_vhbt_fewshot_instances_meta():
    ret = _get_vhbt_instances_meta()
    novel_ids = [k["id"] for k in vhbt_NOVEL_CATEGORIES if k["isthing"] == 1]
    novel_dataset_id_to_contiguous_id = {k: i for i, k in enumerate(novel_ids)}
    novel_classes = [
        k["name"] for k in vhbt_NOVEL_CATEGORIES if k["isthing"] == 1
    ]
    base_categories = [
        k
        for k in vhbt_CATEGORIES
...
def _get_builtin_metadata_vhbt(dataset_name):
    if dataset_name == "vhbt":
        return _get_vhbt_instances_meta()
    elif dataset_name == "vhbt_fewshot":
        return _get_vhbt_fewshot_instances_meta()
