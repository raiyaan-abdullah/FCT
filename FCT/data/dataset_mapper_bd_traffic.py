...
__all__ = ["DatasetMapperWithSupportvhbt"]


class DatasetMapperWithSupportvhbt:
    """
        ...
        if self.is_train:
            ...
            if self.few_shot:
                if 'full' in cfg.DATASETS.TRAIN[0]:
                    if self.seeds == 0:
                        self.support_df = pd.read_pickle("./datasets/vhbt/full_class_{}_shot_support_df.pkl".format(cfg.INPUT.FS.SUPPORT_SHOT))
                        print("training support_df=", "./datasets/vhbt/full_class_{}_shot_support_df.pkl".format(cfg.INPUT.FS.SUPPORT_SHOT))
                    else:
                        self.support_df = pd.read_pickle("./datasets/vhbt/bdtrafficsplit/seed{}/full_class_{}_shot_support_df.pkl".format(self.seeds, cfg.INPUT.FS.SUPPORT_SHOT))
                        print("training support_df=", "./datasets/vhbt/bdtrafficsplit/seed{}/full_class_{}_shot_support_df.pkl".format(self.seeds, cfg.INPUT.FS.SUPPORT_SHOT))
                else:
                    if self.seeds == 0:
                        self.support_df = pd.read_pickle("./datasets/vhbt/{}_shot_support_df.pkl".format(cfg.INPUT.FS.SUPPORT_SHOT))
                        print("training support_df=", "./datasets/vhbt/{}_shot_support_df.pkl".format(cfg.INPUT.FS.SUPPORT_SHOT))
                    else:
                        self.support_df = pd.read_pickle("./datasets/vhbt/bdtrafficsplit/seed{}/{}_shot_support_df.pkl".format(self.seeds, cfg.INPUT.FS.SUPPORT_SHOT))
                        print("training support_df=", "./datasets/vhbt/bdtrafficsplit/seed{}/{}_shot_support_df.pkl".format(self.seeds, cfg.INPUT.FS.SUPPORT_SHOT))
            else:
                self.support_df = pd.read_pickle("./datasets/vhbt/train_support_df.pkl")
                print("training support_df=./datasets/vhbt/train_support_df.pkl")

            metadata = MetadataCatalog.get('vhbt-train')
    ...
    def generate_support(self, dataset_dict):
        ...
        for shot in range(support_shot):
            ...
            support_data = utils.read_image('./datasets/vhbt/images/' + support_db["file_path"].tolist()[0], format=self.img_format)
        ...
        if support_way == 1:
            ...
            for way in range(support_way-1):
                ...
                for shot in range(support_shot):
                    ...
                    support_data = utils.read_image('./datasets/vhbt/' + support_db["file_path"].tolist()[0], format=self.img_format)
                    support_data = torch.as_tensor(np.ascontiguousarray(support_data.transpose(2, 0, 1)))
                    support_box = support_db['support_box'].tolist()[0]
                    support_data_all[mixup_i] = support_data
                    support_box_all[mixup_i] = support_box
