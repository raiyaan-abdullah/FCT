...
CLASS_NAMES = [
    ['rickshaw','cng', 'manual-van']
]

class VhbtEvaluator(DatasetEvaluator):
    """
    ...
    def __init__(self, dataset_name, cfg, distributed, output_dir=None):
        ...
        if not hasattr(self._metadata, "json_file"):
            self._logger.info(
                f"'{dataset_name}' is not registered by `register_vbtv_instances`."
                " Therefore trying to convert it to COCO format ..."
            )

            cache_path = os.path.join(output_dir, f"{dataset_name}_vbtv_format.json")
    ...
    def evaluate(self):
        ...
        if len(predictions) == 0:
            self._logger.warning("[vhbtEvaluator] Did not receive valid predictions.")
            return {}
    ...
    def _eval_predictions(self, tasks, predictions):
        ...
        if self._output_dir:
            file_path = os.path.join(self._output_dir, "vbtv_instances_results.json")
...
def _evaluate_predictions_on_coco(coco_gt, coco_results, iou_type, kpt_oks_sigmas=None):
    ...
    if iou_type == "keypoints":
        ...
        assert num_keypoints_oks == num_keypoints_dt == num_keypoints_gt, (
            f"[VbtvEvaluator] Prediction contain {num_keypoints_dt} keypoints. "
